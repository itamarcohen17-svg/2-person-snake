 ...
 * (Handler_Mouse / Handler_Keyboard) ניגשים אליהם ישירות.
 */
public class GamePanel extends JPanel {
    ...
    final GameSession    session    = new GameSession(sound, highScores, this::repaint);
    ...
    final Renderer_Game     gameRender     = new Renderer_Game();
    ...
    private void paintPlaying(Graphics g) {
        ...
        gameRender.drawGame(g, session, cs, ox, oy,
                settings.getSnake1Color(), settings.getSnake2Color());

        if (session.getState() == GameState.COUNTDOWN)
            gameRender.drawCountdown(g, ox, oy, bp, session.getPreCount());
